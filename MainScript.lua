...
task.spawn(function()
	local success, assetver = pcall(function() return getFromGithub("assetsversion.txt") end)
	if not isfile("flash/assetsversion.txt") then writefile("flash/assetsversion.txt", "0") end
...
task.spawn(function()
	...
	task.spawn(function()
		task.wait(15)
		if image and image.ContentImageSize == Vector2.zero and (not errorPopupShown) and (not redownloadedAssets) and (not isfile("flash/assets/check3.txt")) then 
            errorPopupShown = true
	...
	if scaledgui and scaledgui.Visible then
        ...
        if TextGUI.GetCustomChildren().Parent then
            if (TextGUI.GetCustomChildren().Parent.Position.X.Offset + TextGUI.GetCustomChildren().Parent.Size.X.Offset / 2) >= (gameCamera.ViewportSize.X / 2) then
                ...
                flashText.Position = UDim2.new(1, -158, 0, (flashLogo.Visible and (TextGUIBackgroundToggle.Enabled and 41 or 35) or 5) + (flashCustomText.Visible and 25 or 0) - 23)
                ...
                flashText.Position = UDim2.fromOffset(6, (flashLogo.Visible and (TextGUIBackgroundToggle.Enabled and 41 or 35) or 5) + (flashCustomText.Visible and 25 or 0) - 23)
				flashCustomText.Position = UDim2.fromOffset(0, flashLogo.Visible and 35 or 0)
				flashCustomText.TextXAlignment = Enum.TextXAlignment.Left
		...
		if TextGUIMode.Value == "Drawing" then 
			...
			for i,v in pairs(backgroundList) do 
				...
				textdraw.ZIndex = 2
				textdraw.Position = flashText.AbsolutePosition + Vector2.new(flashText.TextXAlignment == Enum.TextXAlignment.Right and (flashText.AbsoluteSize.X - textdraw.TextBounds.X), ((textdraw.Size - 3) * i) + 6)
        ...
        for i,v in pairs(backgroundList) do
            ...
            backgroundLineFrame.Position = (flashBackgroundList.HorizontalAlignment == Enum.HorizontalAlignment.Left and UDim2.new() or UDim2.new(1, -2, 0, 0))
...
flashScale:GetPropertyChangedSignal("Scale"):Connect(function()
	...
	childrenobj.Position = UDim2.new((check and -(flashScale.Scale - 1) or 0), (check and 0 or -6 * (flashScale.Scale - 1)), 1, -6 * (flashScale.Scale - 1))
	TextGUIUpdate()
end)
	...
	Function = function(val)
		...
		if val == "Drawing" then
			...
			table.insert(TextGUIConnections, flashText:GetPropertyChangedSignal("AbsolutePosition"):Connect(function()
				...
				for i,textdraw in pairs(TextGUIObjects.ShadowLabels) do 
					textdraw.Position = Vector2.new(1, 1) + (flashText.AbsolutePosition + Vector2.new(flashText.TextXAlignment == Enum.TextXAlignment.Right and (flashText.AbsoluteSize.X - textdraw.TextBounds.X), ((textdraw.Size - 3) * i) + 6))
				end
			end))
...
GuiLibrary.UpdateUI = function(h, s, val, bypass)
	pcall(function()
		...
		flashLogoGradient.Color = ColorSequence.new({
			...
			ColorSequenceKeypoint.new(1, flashText.TextColor3)
		})
		flashLogoGradient2.Color = ColorSequence.new({
